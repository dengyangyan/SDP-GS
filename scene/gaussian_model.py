...
class GaussianModel:
    ...
    def __init__(self, args):
        ...
        self._features_dc_language = torch.empty(0)
    ...
    def restore(self, model_args, training_args, mode='train'):
        if len(model_args) == 15: # 这是一个feature训练时保存的ckpt
            ...
            opt_dict, 
            self.spatial_lr_scale,
            self.confidence) = model_args
            print("load model with language feature\n")
            ...
            print("load model without language feature\n")
    ...
    @property
    def get_features_language(self):
        features_dc = self._features_dc_language
        # features_rest = self._features_rest
        # return torch.cat((features_dc, features_rest), dim=1)
        return features_dc.unsqueeze(2) 
    @property
    ...
    def construct_list_of_attributes(self):
        ...
        if self._language_feature is not None:
            for i in range(self._language_feature.shape[1]):
                l.append('languagefeature_{}'.format(i))
        return l
    ...
    def save_fused_ply(self, path):
        ...
        current_opacity_with_filter = self._opacity
    ...
    def prune_points(self, mask, iter, include_feature):
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_language_feature, new_opacities, new_scaling, new_rotation, include_feature):
    ...
    def proximity(self, scene_extent, include_feature, N = 3):
        ...
        if include_feature:
            new_language_feature = self._language_feature[new_indices]
            self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_language_feature, new_opacity, new_scaling, new_rotation, include_feature)
        else:
            self.densification_postfix(new_xyz, new_features_dc, new_features_rest, None, new_opacity, new_scaling, new_rotation, include_feature)


    def densify_and_split(self, grads, grad_threshold, scene_extent, iter, include_feature=False, N=2):
        ...
        self.prune_points(prune_filter, iter, include_feature)

    def densify_and_clone(self, grads, grad_threshold, scene_extent, include_feature):
        # Extract points that satisfy the gradient condition
        ...
        if not include_feature:
            ...
            self.densification_postfix(new_xyz, new_features_dc, new_features_rest, None, new_opacities, new_scaling, new_rotation, include_feature)
        else:
            new_xyz = self._xyz[selected_pts_mask]
            new_features_dc = self._features_dc[selected_pts_mask]
            new_features_rest = self._features_rest[selected_pts_mask]
            ...
            self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_language_feature, new_opacities, new_scaling, new_rotation, include_feature)

    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iteration, include_feature=False):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.densify_and_clone(grads, max_grad, extent, include_feature)
        self.densify_and_split(grads, max_grad, extent, iteration, include_feature)
        
        if iteration < 2000:
            self.proximity(extent, include_feature)
        ...
        self.prune_points(prune_mask, iteration, include_feature)
    ...
    def clone_from_mask(self, selected_pts_mask, repeat=1, include_feature=True):
    ...
    def split_from_mask(self, selected_pts_mask, iter, N=2, repeat=1, include_feature=True):
        for i in range(repeat):
            stds = self.get_scaling[selected_pts_mask].repeat(N, 1)
            means = torch.zeros((stds.size(0), 3), device="cuda")
            samples = torch.normal(mean=means, std=stds)
        ...
        self.prune_points(prune_filter, include_feature)
    ...
    def prune_from_mask(self, prune_mask, iter):
        self.prune_points(prune_mask, include_feature=True)
