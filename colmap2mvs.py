...
if __name__ == '__main__':
    ...
    parser.add_argument('--dense_folder', type=str, help='Project dir.', default='/home/dengyangyan/code/data/nerf_llff_data/trex')
    ...
    renamed_dir = os.path.join(args.dense_folder, 'images_rename')
    ...
    for i in range(num_images):
        cam_txt_name = images[i+1].name[:-4] + '_cam.txt'
        with open(os.path.join(cam_dir, cam_txt_name), 'w') as f:
            f.write('extrinsic\n')
            for j in range(4):
                for k in range(4):
    ...
    #         cv2.imwrite(os.path.join(renamed_dir, images[i+1].name), img)
    #     else:
    #         shutil.copyfile(os.path.join(image_dir, images[i+1].name), os.path.join(renamed_dir, images[i+1].name))